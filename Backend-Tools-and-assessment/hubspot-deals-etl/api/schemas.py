"""
Marshmallow schemas for HubSpot Deals Extraction API
...
class ScanConfigSchema(Schema):
    ...
    type = fields.List(
        fields.Str(validate=validate.OneOf(['hubspot_deals', 'deals', 'user'])),
        required=True,
        validate=validate.Length(min=1),
        error_messages={
            'required': 'Type is required',
            'validator_failed': 'Type must contain at least one value'
        }
    )
    auth = fields.Nested(
        AuthSchema,
    ...
    filters = fields.Nested(FiltersSchema, load_default=dict, dump_default=dict)
